...
def get_column_compact_list(load_dict_item):
    # index start from -1
    max_idx = load_dict_item[-1][0] #表最大idex
    compact_list = [[] for i in range(max_idx+2)] #一共有多少表
    for item in load_dict_item:
        compact_list[item[0]+1].append(item)  #[[*],["perpetrator id",..., "injured"]["people id",...]]
    return compact_list
...
def modify_tables_spider(load_dict):
    for idx, item in enumerate(load_dict):
        if item["db_id"] == "store_1":
            ...
            diff_compact_list.insert(2, pop_item) # 0,1,2->0,2,1
